...
void levelChange() //刘可 
{
	int lev[5] = {0,50,200,500,1000};
    if(score >= lev[0] && score < lev[1]) level = 1;
    else if(score >= lev[1] && score < lev[2]) level = 2;
    else if(score >= lev[2] && score < lev[3]) level = 3;
    else if(score >= lev[3] && score < lev[4]) level = 4;
    else level = 5;    
...
void snakeMove() //王乾 
{
	...
	if(status == U) //向上走
	{
		nexthead->x = head->x; //向上前进时，x坐标不变，y坐标减1 
		nexthead->y = head->y - 1;
		nexthead->next = head;
		head = nexthead;
		q = head;
		...
		if(nexthead->x == food->x && nexthead->y == food->y)
		{
			while(q != NULL)
			{
				...
				color(14);
				printf("★"); //原来食物的位置换成蛇身
				q = q->next; //指针q的下一位也要进行循环操作 
			}
			score = score + add; //吃到食物在总分上加上食物的分 
			speedUp();
			createFood();
		} 
		else
...
void keyboardControl() //王乾 
{
	while(1)
	{
		...
		{
			status = U; //如果蛇不是向下前进的时候，按上键，执行向上的操作 
		}
		else
		{
			if(GetAsyncKeyState(VK_DOWN) && status != U)
			{
				status = D; //如果蛇不是向上前进，按下键，执行向下操作 
			}
			else
			{
				...
				else
				{
					...
					{
						status = R; //如果蛇不是向左前进，按右键，执行向右前进的操作 
					}
				}
		    }
		} 
		...
		else
		{
			...
			{
			  	if(GetAsyncKeyState(VK_F1)) //按F1键，加速
				{
				  	speedUp();
				} 
				else
				{
				   	if(GetAsyncKeyState(VK_F2)) //按F2键，减速
					{
					   	speedDown();
					} 
				}
