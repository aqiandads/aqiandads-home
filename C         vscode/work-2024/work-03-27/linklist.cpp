...
//头插法建立单链表
{
	LinkNode *s; 	//创建头结点(头结点在main函数中创建 L=(LinkNode *)malloc(sizeof(LinkNode)))
	L->next=NULL;
	int i;
	for (i=0;i<n;i++)
	{	
...
//尾插法建立单链表
{
	LinkNode *s,*r; 	//创建头结点(头结点在main函数中创建 L=(LinkNode *)malloc(sizeof(LinkNode)))
	L->next=NULL;
	r=L;
	int i;						//r始终指向尾结点,开始时指向头结点
	for (i=0;i<n;i++)
	{	
		s=(LinkNode *)malloc(sizeof(LinkNode));//创建新结点s
		s->data=a[i];
